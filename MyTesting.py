...
for _data_name in [opt.test_path]:
    ...
    for i in range(test_loader.size):
        ...
        os.makedirs(save_path+"/out4out3final", exist_ok=True)
        ...
        os.makedirs(save_path+"/out2out4out3final", exist_ok=True)
        ...
        os.makedirs(save_path+"/out1out2out4out3final", exist_ok=True)
        res = F.upsample(P1[0] + P1[1] + P1[2] + P1[3] + P2, size=gt.shape, mode='bilinear', align_corners=False)
        res = res.sigmoid().data.cpu().numpy().squeeze()
        res = (res - res.min()) / (res.max() - res.min() + 1e-8)
        print('> {} - {}'.format(_data_name, name))
        cv2.imwrite(save_path+"/out1out2out4out3final/"+name,res*255)
