...
for _data_name in [opt.test_path]:
    ...
    for i in range(test_loader.size):
        ...
        cv2.imwrite(save_path+"/aivgnet0123/"+name,res*255)
        ...
        res = (res - res.min()) / (res.max() - res.min() + 1e-8)
        print('> {} - {}'.format(_data_name, name))
        cv2.imwrite(save_path+"/aivgnet123/"+name,res*255)
        ...
        res = (res - res.min()) / (res.max() - res.min() + 1e-8)
        print('> {} - {}'.format(_data_name, name))
        cv2.imwrite(save_path+"/aivgnet23/"+name,res*255)
        ...
        res = (res - res.min()) / (res.max() - res.min() + 1e-8)
        print('> {} - {}'.format(_data_name, name))
        cv2.imwrite(save_path+"/aivgnet3/"+name,res*255)
        ...
        res = (res - res.min()) / (res.max() - res.min() + 1e-8)
        print('> {} - {}'.format(_data_name, name))
        cv2.imwrite(save_path+"/aivgnet/"+name,res*255)
        
        os.makedirs(save_path+"/aivgnet0", exist_ok=True)
        res = F.upsample(P1[0], size=gt.shape, mode='bilinear', align_corners=False)
        res = res.sigmoid().data.cpu().numpy().squeeze()
        res = (res - res.min()) / (res.max() - res.min() + 1e-8)
        print('> {} - {}'.format(_data_name, name))
        cv2.imwrite(save_path+"/aivgnet0/"+name,res*255)
